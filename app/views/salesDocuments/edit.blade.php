...
{{ Form::open(array('url'=>'salesDocuments/'.$salesDocument->id,'method' => 'put', 'class'=>'form-horizontal')) }}
<table class="table table-striped table-bordered table-hover table-condensed">
  <thead>
    <tr>
      <th> Line# </th>
  ...
  @if(Auth::user()->supplierID==0 ||($salesDocumentItem->productID!=0&&$salesDocumentItem->product->supplierID == Auth::user()->supplierID))
      <tr>
        ...
        @if($salesDocumentItem->product!=null)
	        ...
	        <td> 
	        {{ Form::hidden('salesDocumentItemID['.$index.']', $salesDocumentItem->id) }}
	        {{ Form::select('deliveryTypeID['.$index.']', $deliveryTypes, $salesDocumentItem->deliveryTypeID , array('class'=>'form-control danger')) }}
	        </td>
        @else
	        <td>
	        	{{ $salesDocumentItem->itemName }} <br/>
