...
<table  class="table table-striped table-bordered table-hover table-condensed">
  <tbody>
    <tr>
      ...
      <td> {{ $supplierSalesDocument->salesDocument->type }} </td>
    </tr>
    <tr>
      <td><label> Source  </label></td>
      <td> {{ $supplierSalesDocument->salesDocument->source }} </td>
      <td><label> Warehouse Name  </label></td>
      <td> {{ $supplierSalesDocument->salesDocument->warehouseName }} </td>
      <td><label> Delivery  </label></td>
      <td> {{ $supplierSalesDocument->status }} </td>
    </tr>
    <!-- <tr>
      <td><label> Invoice State  </label></td>
      <td> {{ $supplierSalesDocument -> invoiceState }} </td>
      <td><label> PaymentType  </label></td>
      <td> {{ $supplierSalesDocument -> paymentType }} </td>
      <td><label> paymentStatus  </label></td>
      <td> {{ $supplierSalesDocument -> paymentStatus }} </td>
    </tr> -->
    <tr>
      <td><label> Custoemr Name  </label></td>
      <td> {{ $supplierSalesDocument->salesDocument->clientName }} </td>
      <td><label> Customer Email  </label></td>
      <td> {{ $supplierSalesDocument->salesDocument->clientEmail }} </td>
      <td><label> Notes  </label></td>
      <td><span data-toggle="tooltip" title="Customer Notes">{{ $supplierSalesDocument -> notes }} </span>
          <span data-toggle="tooltip" title="Internal Notes">{{ $supplierSalesDocument -> internalNotes }}</span>
      </td>
    </tr>
    <tr>
      <td><label> Address  </label></td>
      <td colspan="5"> {{ $supplierSalesDocument->salesDocument->address }} </td>
    </tr>
    <!-- <tr>
      ...
      <td> {{ $supplierSalesDocument -> rounding }} </td>
    </tr>
    <tr>
      ...
      <td> {{ $supplierSalesDocument -> paid }} </td>
      <td><label> cost  </label></td>
      <td> {{ $supplierSalesDocument -> cost }} </td>
      
    </tr> -->
    <tr>
      <td><label> Create Date  </label></td>
      <td> {{ $supplierSalesDocument->salesDocument->added }} </td>
...
<table class="table table-striped table-bordered table-hover table-condensed">
  ...
  <tbody>
  @foreach($supplierSalesDocument->supplierSalesDocumentItems() as $index=>$salesDocumentItem )
  ...
  <!-- DisplayTotal Value -->
      <tr class="">
        <td colspan="6"> </td>
        <td><strong>{{$supplierSalesDocument->amount}}</strong></td>
        <td colspan="2"></td>
        <td class="text-right"> <strong>{{ $supplierSalesDocument->netTotal  }} </strong></td>
        <td class="text-right"><strong> {{ $supplierSalesDocument->vatTotal }}</strong></td>
        <td class="text-right"><strong> {{ $supplierSalesDocument->total }}  </strong></td>
        <td></td>
      </tr>
  </tbody>
</table>
